"""K12+K13+K16 Provenance-Integration fuer df-cape-coral-property-tracker [CRUX-MK].
...
_HMAC_SECRET = os.environ.get("DF_CCPT_HMAC_SECRET", "df-cape-coral-property-tracker-dev-hmac-v1")
_TTL_S = int(os.environ.get("DF_CCPT_ENVELOPE_TTL_S", "86400"))
DEFAULT_LOCK_PATH = Path("/tmp/df-cape-coral-property-tracker.lock.lockfile")


class CapeCoralPropertyProvenanceRecorder:
    """K11 try/except + K12 envelope + K13 RFC3161-anchor + K16 AtomicLock."""

    def __init__(self, audit_dir: Path | str = "branch-hub/audit/df-cape-coral-property-tracker/",
    ...
    def record_action(self, action_type: str, payload: dict[str, Any]) -> Optional[dict]:
        ...
        try:
            ...
            envelope = build_full_envelope(
                operation_id=f"df-cape-coral-property-tracker-{datetime.now(timezone.utc).isoformat()}",
                operation_type=action_type,
                issuer="df-cape-coral-property-tracker",
                payload=payload,
